...
test.describe('[QE-449][AC1] Desktop Viewport Tests', { tag: ['@regression', '@princess-cruises'] }, () => {
  let alaskaPage;

  test('[QE-507] Verify BOOK YOUR VACATION button is visible and clickable on desktop viewport', async ({ page }) => {
    alaskaPage = new PrincessAlaskaPage(page);

    // Step 1: Set browser viewport to desktop dimensions
    await alaskaPage.setViewportSize(TD.viewports.desktop.width, TD.viewports.desktop.height);

    // Step 2: Navigate to Alaska destination landing page
    await alaskaPage.goto();
    await expect(page).toHaveURL(TD.urls.alaskaLanding);

    // Step 3: Verify hero banner is displayed properly on desktop
    const isHeroBannerVisible = await alaskaPage.isHeroBannerVisible();
    expect(isHeroBannerVisible).toBe(true);

    // Step 4: Verify BOOK YOUR VACATION button is visible
